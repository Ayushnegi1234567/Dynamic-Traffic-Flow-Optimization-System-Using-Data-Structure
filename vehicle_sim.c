...
int total_time_between_nodes(char *path[]) {
    int count = 0;
    while (path[count] != NULL) count++;
    return count + 2; // Simulated time, e.g., 2 seconds per node
}
...
int main() {
    ...
    char *path1[] = {"A", "B", "C", NULL};
    Vehicle_info_struct v1 = {
        .vehicle_type = "Bus",
        .source = "A",
        .destination = "C",
        .id = 1,
    ...
    addVehicleInformation(v1);

    pthread_t display_thread;
    pthread_create(&display_thread, NULL, display_loop, NULL);
    pthread_join(display_thread, NULL); // Keep running
